...
class Movie(db.Model):
    ...
    # optional: gives a cleaner debugging representation in Python.
    def __repr__(self):
        return f"<Movie {self.title}>"
    

# Defining that the model class doesnt automatically create the real database table.
# here we tell Flask-SQLAchemy to create the tables.
# Docs explain that man extension operations require an application context.
with app.app_context():
    # actually builds the table in the database if it doesn't exist yet.
    db.create_all() # We do the Mapping here (create our tables)
